...
; 0001 009C                 }
; 0001 009D                 #if T_STEP > 1
; 0001 009E                 if (T_LoadOn < TLoadOn_Min) //если "проскочили" минимальную температуру, то
; 0001 009F                 {
; 0001 00A0                   T_LoadOn = TLoadOn_Min;      //устанавливаем минимум
; 0001 00A1                 }
; 0001 00A2                 #endif
; 0001 00A3 //                View = SHOW_TLoadOn;           //удерживаем в режиме "Установленная температура"
; 0001 00A4 //                Counter = 5;        //и взводим счётчик на 5 секунд.
; 0001 00A5               break;
_0x20021:
	RJMP _0x2001F
; 0001 00A6               case 2:               //если мы в режиме "Дэльта", то
_0x20020:
	CPI  R30,LOW(0x2)
	BRNE _0x2001F
; 0001 00A7                 if (DeltaT > DeltaT_Min)     //если "Дэльта" больше Min, то
	LDI  R30,LOW(1)
	LDI  R31,HIGH(1)
	CP   R30,R8
	CPC  R31,R9
	BRGE _0x20023
; 0001 00A8                 {
; 0001 00A9                   DeltaT --;        //уменьшаем Дэльту на 0,1°
	MOVW R30,R8
	SBIW R30,1
	MOVW R8,R30
; 0001 00AA //                  RefreshDisplay(); //обновляем данные на экране
; 0001 00AB                 }
; 0001 00AC //                Counter = 5;        //и взводим счётчик ещё на 5 секунд.
; 0001 00AD               break;
_0x20023:
; 0001 00AE               #ifdef CorCode
; 0001 00AF               case 3:                   //если мы в режиме "Коррекции", то
; 0001 00B0                 if (CorT > CorT_Min)
; 0001 00B1                 {
; 0001 00B2                     CorT--;         //уменьшаем значение на 0,1°
; 0001 00B3                 }
; 0001 00B4                 break;
; 0001 00B5               #endif
; 0001 00B6             }
...
; 0001 00B7         break;
	RJMP _0x2001B
; 0001 00B8 
; 0001 00B9         case KEY_2:                 // Была нажата клавиша Плюс
_0x2001C:
	CPI  R30,LOW(0x2)
	BRNE _0x20024
; 0001 00BA             switch (View)
	MOV  R30,R2
; 0001 00BB             {
; 0001 00BC               #ifdef ENTER_SETTINGS_BY_ONE_KEY
; 0001 00BD               case 0:               //если был режим "Текущая температура", то
; 0001 00BE                  View = SHOW_TLoadOn;           //переходим в режим "Установленная температура"
; 0001 00BF               break;
; 0001 00C0               #endif
; 0001 00C1               case SHOW_TLoadOn:               //если мы в режиме "Установленная температура", то
	CPI  R30,LOW(0x1)
	BRNE _0x20028
; 0001 00C2                 if (T_LoadOn < (TLoadOn_Max - DeltaT))    //если температура ниже Max - Дельта
	LDI  R30,LOW(1250)
	LDI  R31,HIGH(1250)
	SUB  R30,R8
	SBC  R31,R9
	...
	BRGE _0x20029
; 0001 00C3                 {
; 0001 00C4                   T_LoadOn += T_STEP;      //то увеличиваем Установленную температуру на 0,1°
	MOVW R30,R6
	ADIW R30,1
	MOVW R6,R30
; 0001 00C5 //                  RefreshDisplay(); //обновляем данные на экране
; 0001 00C6                 }
; 0001 00C7                 #if T_STEP > 1
; 0001 00C8                 if (T_LoadOn > TLoadOn_Max - DeltaT) //если "проскочили" максимальную температуру, то
; 0001 00C9                 {
; 0001 00CA                   T_LoadOn = TLoadOn_Max - DeltaT;      //устанавливаем максимум
; 0001 00CB                 }
; 0001 00CC                 #endif
; 0001 00CD //                View = SHOW_TLoadOn;           //удерживаем в режиме "Установленная температура"
; 0001 00CE //                Counter = 5;        //и взводим счётчик ещё на 5 секунд.
; 0001 00CF               break;
_0x20029:
	RJMP _0x20027
; 0001 00D0               case 2:
_0x20028:
	CPI  R30,LOW(0x2)
	BRNE _0x20027
; 0001 00D1                 if (DeltaT < DeltaT_Max)   //если Дельта меньше Max, то
	LDI  R30,LOW(250)
	LDI  R31,HIGH(250)
	CP   R8,R30
	CPC  R9,R31
	BRGE _0x2002B
; 0001 00D2                 {
; 0001 00D3                   DeltaT ++;        //то увеличиваем Дэльту на 0,1°
	MOVW R30,R8
	ADIW R30,1
	MOVW R8,R30
; 0001 00D4 //                  RefreshDisplay(); //обновляем данные на экране
; 0001 00D5                 }
; 0001 00D6 //                Counter = 5;        //и взводим счётчик ещё на 5 секунд.
; 0001 00D7               break;
_0x2002B:
; 0001 00D8               #ifdef CorCode
; 0001 00D9               case 3:                   //если мы в режиме "Коррекции", то
; 0001 00DA                 if (CorT < CorT_Max)
; 0001 00DB                 {
; 0001 00DC                     CorT++;
; 0001 00DD                 }
; 0001 00DE               break;
...
; 0001 00E1         break;
	RJMP _0x2001B
; 0001 00E2 
; 0001 00E3         case KEY_3:               // Была нажаты обе кноки вместе.
_0x20024:
	CPI  R30,LOW(0x3)
	BRNE _0x2002E
; 0001 00E4           View++;
	INC  R2
; 0001 00E5           if (View > View_Max)
	LDI  R30,LOW(3)
	CP   R30,R2
	BRSH _0x2002D
; 0001 00E6           {
; 0001 00E7             View = SHOW_TLoadOn;
	LDI  R30,LOW(1)
	MOV  R2,R30
; 0001 00E8           }
; 0001 00E9 //             switch (View)
; 0001 00EA //             {
; 0001 00EB //               case 0:               //если был режим "Текущая температура", то
; 0001 00EC //                 View = SHOW_TLoadOn;           //переходим в режим "Установленная температура"
; 0001 00ED //               break;
; 0001 00EE //               case 1:               //если мы в режиме "Установленная температура", то
; 0001 00EF //                 View = SHOW_DeltaT;           //удерживаем в режиме "Дэльта"
; 0001 00F0 //               break;
; 0001 00F1 //               case 2:
; 0001 00F2 //                 View = SHOW_Error;           //переходим в режим "Последняя обнаруженная ошибка"
; 0001 00F3 //               break;
; 0001 00F4 //               case 3:
; 0001 00F5 //                 View = SHOW_TLoadOn;           //переходим в режим "Установленная температура"
; 0001 00F6 //               break;
; 0001 00F7 //             }
; 0001 00F8 //             Counter = 5;        //и взводим счётчик на 5 секунд.
; 0001 00F9 //             Counter = 5;        //и взводим счётчик ещё на 5 секунд.
; 0001 00FA //             Counter = 5;        //и взводим счётчик ещё на 5 секунд.
; 0001 00FB //            Counter = 5;        //и взводим счётчик ещё на 5 секунд.
; 0001 00FC         break;
_0x2002D:
; 0001 00FD 
; 0001 00FE         default:
...
; 0001 0101     }
_0x2001B:
; 0001 0102     Counter = 5;        //и взводим счётчик ещё на 5 секунд.
	LDI  R30,LOW(5)
	MOV  R3,R30
; 0001 0103     #ifdef Blinking
; 0001 0104     GoBlinking = 0;
	CBI  0x14,0
; 0001 0105     #endif
; 0001 0106     RefreshDisplay(); //обновляем данные на экране
	RCALL _RefreshDisplay
; 0001 0107 
; 0001 0108 }
	RET
